...
function renderCards(dataArray){
    ...
    dataArray.forEach(element => {
        ...
        if (indexToRender === -1) {
            htmlSvg = `
                    <svg id="card-plus" data-id="${element.imdbID}" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            htmlSvg = `
                    <svg id="card-plus" data-id="${element.imdbID}" width="16" height="16" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
...
async function handleSearch(){
    if (searchEl.value !== '') {
        const response = await fetch(`http://www.omdbapi.com/?apikey=4d93911c&s=${searchEl.value}`)
        ...
        if (data['Search'] !== 'undefined' && data.Response !== 'False'){
            const completeData = await Promise.all(
                data['Search'].map( async data => {
                    const response = await fetch(`http://www.omdbapi.com/?apikey=4d93911c&i=${data.imdbID}`)
